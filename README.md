...
`wiktionary-teochew-index.yaml` also has one status the Swadesh checklist
doesn't: `no_gloss`, a terminal state a human/agent sets by hand (never the
importer) after confirming a `staged` headword's live Wiktionary page gives no
independently definable content — bound character, compound-only note, or an
unfilled stub — despite carrying a valid reading. Issue #68 batch 20
introduced it once the first 19 merge batches had exhausted every headword
Wiktionary actually defines.
...
1. Fetch its Wiktionary page for a gloss and part of speech (single-reading
   ...
   gloss that the page doesn't support. Trust genuine Chinese-language-section
   content (a standalone sense, an alt-of/alternative-form sense, or a
   dialect-labelled synonym cross-reference — see `dang1-鄲` in
   `data/entries/grammar.yaml`) as sufficient; a generic cross-CJK
   "Translingual" Han-character gloss sitting next to an unfilled
   Chinese-section stub is *not* — issue #68 batch 20 established that this
   dictionary's other single-character entries are all sourced from
   Chinese-specific content, so a Translingual-only gloss doesn't clear the
   same bar and the headword should be treated as step 3b below instead.
2. Add the entry to whichever existing category file fits its meaning
   ...
   proposal's date.
3. Flip the headword's `wiktionary-teochew-index.yaml` entry:
   - **3a.** If it was mergeable: `staged` → `existing` with
     `entry_id: <the new id>` — the same `status` lifecycle
     `swadesh-207.yaml` already uses, so this doubles as the batch's
     resumability marker.
   - **3b.** If its live page genuinely gives no independently definable
     gloss (see step 1): `staged` → `no_gloss` instead — a terminal,
     not-actionable state, so the checklist doesn't keep listing it as a
...
referencing #68 without closing it — until the batch that finds zero
`staged` items left, which closes it.
...
the key are a no-op (exit 0, nothing uploaded or written). A different
recording from a speaker who already has a clip there is appended as a
further, training-only take rather than replacing it — pass `--primary` to
publish the new take instead of the existing one (ADR-0029, issue #290).
`--force` is gone: there is nothing left for it to authorise overwriting.
...
**Multiple takes per speaker (ADR-0029, issue #290).** `clips[<pengim>]` (and
`wordClips`) may hold more than one clip from the same `speaker` — `checksum`
is a clip's identity, `speaker` is a person's. Two optional fields say which
take is published: `take` (an integer ≥ 2; absent means the speaker's first
take at that key) and `primary: true` (exactly one clip in a group of more
than one; a lone clip is implicitly primary):
...
Only the primary clip in each `(pengim, speaker)` group reaches `dist/`, the
web UI, and the ADR-0027 `-n` render tier; the other takes exist for the
held-out classifier eval and are never listenable in the UI. Because a take's
asset path (`<speaker>/<pengim>-take<N>.<ext>`) is immutable, a re-recording
never overwrites the previous take's bytes — no CloudFront invalidation is
...
tones they carry do. `npm run audio:grade` pools each speaker's *primary*
clip only — a speaker's other takes and any `-n` synthesis render are
excluded (ADR-0029), so neither inflates a syllable's weight in the per-tone
medians. It caches every primary clip locally
...
beside each *primary* recording (ADR-0029 — a render's `derivedFrom` must
name a primary, so the `-n` group at a key is always size 1) under speaker id
`<speaker>-n` with
