 ...
 *
 * Pure: every function here takes its `PengimScheme` explicitly rather than
 * loading one from disk. The root project's `src/phonology/syllable.ts` is a
 * thin wrapper around this module that adds a disk-backed default scheme for
 * callers that don't have one already loaded — see that file's doc comment.
 */
