...
import type { PojScheme } from '../schema/phonology.js'
 ...
 *
 * Pure: takes already-parsed syllables and an already-loaded `PojScheme`
 * rather than loading one from disk. The root project's `src/phonology/poj.ts`
 * adds `toPoj`, a disk-backed convenience wrapper around `syllablesToPoj`
 * below — see that file's doc comment.
 */
