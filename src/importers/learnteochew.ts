...
export function parsePronunciationHtml(html: string): ParsedChart {
  ...
  const initialsTable = rest.find((t) => t !== finalsTable)

  if (!initialsTable || !finalsTable || !tonesTable) {
    throw new Error(
      "learnteochew pronunciation page structure has changed — expected Consonants, Finals and Tones tables each with a Peng'im column",
    )
  }
