...
export function writeAudioStaging(result: AudioImportResult): string {
  ...
  const header = [
    ...
    "# it's actually an example of, re-host its bytes (see the rehost CLI), then",
    '# hand-write it into data/phonology/audio/<variety>.yaml — under `clips`',
    '# if syllableCount is 1, `wordClips` if 2 or more (REVIEW.md § 16).',
    '#',
