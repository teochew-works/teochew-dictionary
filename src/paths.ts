...
/** Per-variety audio clip metadata (issue #31): `{id}.yaml` → `AUDIO_FILES_DIR/{id}/`. */
export const AUDIO_METADATA_DIR = join(PHONOLOGY_DIR, 'audio')
/** The actual recorded clips, kept out of `phonology/` since they're binary media, not YAML tables. */
export const AUDIO_FILES_DIR = join(DATA_DIR, 'audio')
