...
it('creates a deck with the selected entry without dragging', () => {
  ...
  const study = screen.getByRole('link', { name: 'Study this deck' })
  expect(study).toHaveAttribute('href', '#flashcards')
  fireEvent.click(study)
  expect(readDecksState().inPlay).toEqual([saved.decks[0]!.id])
})
